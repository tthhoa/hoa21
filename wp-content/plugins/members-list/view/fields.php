<div class="wrap">
	<h1>Display Fields <a href="#TB_inline?width=400&height=700&inlineId=members_list_add_item" id="add_item" class="thickbox page-title-action">Add New</a></h1>
	...
	<?php if(isset($o['fields']) and !empty($o['fields'])) { ?>
	<form id="tern_wp_members_list_fm" method="post" action="<?php bloginfo('wpurl'); ?>/wp-admin/admin.php?page=members-list-configure-search">
		...
		<table id="members_list_fields" class="widefat fixed" cellspacing="0">
			...
			<tbody id="fields" class="list:sort sort-list">
				<?php foreach((array)$o['fields'] as $k => $v) { $d = empty($d) ? ' class="alternate"' : ''; ?>
				<tr id="list-<?php echo $v; ?>"<?php echo $d; ?>>
					<th scope="row" class="check-column">
						<input type="checkbox" name="fields[]" value="<?php echo $v; ?>" />
					...
					<td><strong><?php echo $k; ?></strong><br />
						<div class="row-actions">
							<span class='edit'> <a href="admin.php?page=members-list-configure-fields&fields%5B%5D=<?php echo $v; ?>&action=delete&_wpnonce=<?php echo wp_create_nonce('tern_wp_members_nonce'); ?>">Delete</a> </span>
						</div></td>
					<td><?php echo $v; ?></td>
				</tr>
				<?php } ?>
		...
		<input type="hidden" name="action" value="order" /> <input type="hidden" name="page" value="members-list-configure-fields" /> <input type="hidden" id="_wpnonce" name="_wpnonce" value="<?php echo wp_create_nonce('tern_wp_members_nonce'); ?>" />
	</form>
	<?php } ?>
</div>
